      ...
      const battlemanager = {
        ...
        spellcast: function(casterName, spellName, targetUnitId){
            ...
            if(typeof commander === 'undefined'){
                throw Error(`Can\'t cast whit non-existant commander ` + casterName + '!');
            }
            const spell = commander
            .spellbook
        ...
        battle: (function() {
          ...
          .forEach( unit => properties.forEach(p => {
            ...
            const totalGamage = atacker.damage * atacker.count;
            const totalHealth = defender.health*defender.count;
            const newCount = defender.count = Math.ceil((totalHealth - totalDamage)/ defender.health);
